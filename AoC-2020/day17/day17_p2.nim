...
proc read_the_cell_at_the_beginning: seq[Cell]=
    ...
    while true:
        ...
        for i, each_char in input_line:
            if each_char == '#':
                result.add( Cell(x:i, y:init_y, z:0, w:0))
        init_y += 1

        echo(input_line, ret)

proc Does_cell_active(cur_cells: seq[Cell], x:int, y:int, z:int, w:int):bool =
    for each_cell in cur_cells:
        if (each_cell.x == x) and (each_cell.y == y) and (each_cell.z == z) and (each_cell.w == w):
            return true
    return false

proc Count_neighbor_cell(cur_cells: seq[Cell], x:int, y:int, z:int, w:int):int = 
    for dx in countup(-1, 1):
        for dy in countup(-1, 1):
            for dz in countup(-1, 1):
                for dw in countup(-1, 1):
                    if not ((dx == 0) and (dy == 0) and (dz == 0) and (dw == 0)):
                        result += int(Does_cell_active(cur_cells , x + dx, y + dy, z+dz, w + dw))
                    
proc max_ranges(cur_cells: seq[Cell]): tuple[x0:int, x1:int, y0:int, y1:int, z0:int, z1:int, w0:int, w1:int] = 
    ...
    result.w0 = cur_cells[0].w
    result.w1 = cur_cells[0].w
    ...
    for each_cell in cur_cells:
        ...
        if  result.w0 > each_cell.w:
            result.w0 = each_cell.w 
        if  result.w1 < each_cell.w:
            result.w1 = each_cell.w
    ...
    for new_x in countup(next_possible_range.x0, next_possible_range.x1):
        for new_y in countup(next_possible_range.y0, next_possible_range.y1):
            for new_z in countup(next_possible_range.z0, next_possible_range.z1):
                for new_w in countup(next_possible_range.w0, next_possible_range.w1):
                    var counted_neightbors = Count_neighbor_cell(prev_state, new_x, new_y, new_z, new_w)

                    if Does_cell_active(prev_state, new_x, new_y, new_z, new_w):
                        if (counted_neightbors==2) or (counted_neightbors==3):
                            result.add(Cell(x:new_x, y:new_y, z:new_z, w:new_w))
                    else:
...
proc count_active_cells(prev_state: seq[Cell]): int=
    ...
    for new_x in countup(next_possible_range.x0, next_possible_range.x1):
        for new_y in countup(next_possible_range.y0, next_possible_range.y1):
            for new_z in countup(next_possible_range.z0, next_possible_range.z1):
                for new_w in countup(next_possible_range.w0, next_possible_range.w1):
                    result += int(Does_cell_active(prev_state, new_x, new_y, new_z, new_w))
