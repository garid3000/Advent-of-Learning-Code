...
func follow_the_lead(lead_coor, follower_coor Coord) Coord{
	...
	if (abs(ver_dis) < 2) && (abs(hor_dis) < 2) {
		...
		if ver_dis > 0 {newFollower_coor.y += 1}
		if ver_dis < 0 {newFollower_coor.y -= 1}
		return newFollower_coor
	} else if (abs(hor_dis) == 2) && (ver_dis == 0) {
		//only move horizontally
		if hor_dis > 0 {newFollower_coor.x += 1}
		if hor_dis < 0 {newFollower_coor.x -= 1}
		return newFollower_coor
	} else {
		//now the diagonale
		...
		if is_tail_touched_head(lead_coor, tmp_coor0) {return tmp_coor0}
		if is_tail_touched_head(lead_coor, tmp_coor1) {return tmp_coor1}
		if is_tail_touched_head(lead_coor, tmp_coor2) {return tmp_coor2}
		if is_tail_touched_head(lead_coor, tmp_coor3) {return tmp_coor3}
...
func log_maxmin_of_head(){
	if chain[0].x > maxheadx {maxheadx = chain[0].x}
	if chain[0].x < minheadx {minheadx = chain[0].x}
	if chain[0].y > maxheady {maxheady = chain[0].y}
	if chain[0].y < minheady {minheady = chain[0].y}
}
